...
class PreloadAsset extends React.Component {
	...
	resize() {
		...
		if (this.props.size === PreloadAsset.SIZE.CONTAIN) {
			size = new BackgroundTools.contain(elementBounds.width, elementBounds.height, containerBounds.width, containerBounds.height);
		}
		else {
			size = new BackgroundTools.cover(elementBounds.width, elementBounds.height, containerBounds.width, containerBounds.height);
		}
